...
위의 문제를 해결하기 위해 ```Delayed Scheduler```, ```Greedy Scheduler``` 두 가지 방식을 논문에선 제안한다. 논문에서는 전체 점 중 일부만 재배열해도 충돌이 발생하지 않는다고 한다.

먼저, ```Delayed Scheduler``` 방식이다.  
본 방식은 충돌하는 점들을 뒤로 미루고, 다음 루프에서 처리하는 방식이다. 충돌난 점들만 다시 시도하는 것이라, 또 충돌날 확률은 상당히 적어진다고 한다. 단점은, 충돌난 점들을 저장해야 하기에, $$ NT/2^{c-1} $$ 개의 점과 Scalar를 따로 저장해주어야 한다.

다음으로, ```Greedy Scheduler``` 방식이다.  
...
$$ p_1 &= a, \quad p_2 = ab, \quad p_3 = abc \\
z &= (abc)^{-1} \\
c^{-1} &= z(ab) = zp_2 \\
b^{-1} &= p_1cz \\
a^{-1} &= bcz $$